...
class HomePage(_PageSingleton):
    ...
    directions_section_title = models.CharField('Напрями — заголовок розділу', max_length=200, blank=True,
                                                default='Три напрями — одна компанія')
    # Про компанію
    about_title = models.TextField('Про нас — заголовок', blank=True,
                                   default='ТОВ «ФортунаПринт» — ваш партнер у сфері маркування та контролю якості')
    about_body_1 = models.TextField('Про нас — абзац 1', blank=True,
                                    default='Ми постачаємо та впроваджуємо промислове обладнання для виробничих підприємств харчової, фармацевтичної та косметичної галузей.')
    about_body_2 = models.TextField('Про нас — абзац 2', blank=True,
                                    default='Наші рішення відповідають вимогам HACCP та міжнародним стандартам якості. Від підбору обладнання до технічного обслуговування — ми супроводжуємо клієнта на кожному етапі.')
    # Stats
...
class MarkingPage(_PageSingleton):
    ...
    hero_subtitle = models.TextField('Hero — підзаголовок', blank=True,
                                     default='Лазерне нанесення штрих-кодів, QR-кодів, серійних номерів, логотипів та дат виробництва на будь-яких матеріалах — метал, пластик, скло, дерево, шкіра.')
    # Intro
