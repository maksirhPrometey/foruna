...
if ! python manage.py shell -c "from django.contrib.auth import get_user_model; exit(0 if get_user_model().objects.filter(is_superuser=True).exists() else 1)" 2>/dev/null; then
    echo ""
...
echo "   cd ${APP_DIR} && source .venv/bin/activate && gunicorn config.wsgi:application --bind 127.0.0.1:3000"
echo ""
